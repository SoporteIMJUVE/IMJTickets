...
<div class="p-8 max-w-2xl mx-auto">
    ...
    <div class="flex items-center gap-4 mb-8">
        <a href="{{ route('tickets.index') }}"
           class="p-2 rounded-lg hover:bg-[#eae8e7] transition-colors text-[#544246] hover:text-[#621132]">
            <span class="material-symbols-outlined">arrow_back</span>
        </a>
        <div>
            <h2 class="text-[28px] font-bold leading-tight tracking-tight text-[#621132]">Nuevo Ticket</h2>
            <p class="text-[#544246] text-sm mt-0.5">Registra una incidencia de soporte técnico.</p>
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('tickets.store') }}" novalidate>
        ...
        <div class="bg-white border border-[#E5E7EB] rounded-2xl shadow-sm overflow-hidden">
            ...
            <div class="px-6 pt-6 pb-5 border-b border-[#F3F4F6] flex items-center gap-3">
                <div class="w-9 h-9 rounded-full bg-[#D4C19C]/40 flex items-center justify-center text-[#621132] font-bold text-sm shrink-0">
                    {{ strtoupper(substr(auth()->user()->name ?? 'U', 0, 2)) }}
                </div>
                <div>
                    <p class="text-[11px] font-bold uppercase tracking-wider text-[#544246]">Solicitante</p>
                    <p class="text-sm font-semibold text-[#1b1c1c]">{{ auth()->user()->name }}</p>
                    <p class="text-xs text-[#544246]">{{ $autofillCorreo }}</p>
                </div>
            </div>
        ...
        <div class="mt-6 flex items-center justify-end gap-3">
            <div class="flex items-center gap-3">
                <a href="{{ route('tickets.index') }}"
                   class="px-5 py-2.5 rounded-lg text-sm font-bold text-[#544246] hover:bg-[#eae8e7] transition-colors">
                    Cancelar
                </a>
                <button type="submit"
