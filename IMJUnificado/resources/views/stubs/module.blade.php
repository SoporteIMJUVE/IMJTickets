...
<div class="p-8" id="modulo-page">
    ...
    {{-- ══════════════════════════════════════════════════════════════
         ...
         - h2: nombre del módulo (siempre en guinda var(--color-brand))
    ...
    <div class="flex justify-between items-end mb-6">
        <div>
            <h2 class="text-[32px] font-bold leading-10 tracking-tight text-brand">Nombre del Módulo</h2>
            <p class="text-muted text-sm mt-1">Descripción breve del propósito de este módulo.</p>
        </div>
    ...
    <div class="grid grid-cols-3 gap-6 mb-8">
        <div class="bg-canvas p-6 rounded-xl border border-border shadow-sm">
            <p class="text-[11px] font-bold uppercase tracking-wider text-muted mb-2">Métrica 1</p>
            <h3 class="text-3xl font-bold text-brand">0</h3>
        ...
        <div class="bg-canvas p-6 rounded-xl border border-border shadow-sm">
            <p class="text-[11px] font-bold uppercase tracking-wider text-muted mb-2">Métrica 2</p>
            <h3 class="text-3xl font-bold text-brand">0</h3>
            <div class="mt-4 h-2 bg-wash rounded-full overflow-hidden">
                <div class="h-full bg-gold rounded-full" style="width:0%"></div>
            </div>
        </div>
        <div class="bg-canvas p-6 rounded-xl border border-border shadow-sm">
            <p class="text-[11px] font-bold uppercase tracking-wider text-muted mb-2">Métrica 3</p>
            <h3 class="text-3xl font-bold text-status-critical">0</h3>
            ...
            >
                ...
                <x-slot:acciones>
                    <button onclick="abrirModalNuevo()"
                            class="px-4 py-2 bg-brand text-white rounded flex items-center gap-2 hover:opacity-90 transition-opacity text-sm font-bold">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input id="f-modulo-texto" type="text" oninput="filtrarTabla()"
                                   ...
                                   class="pl-7 pr-3 py-1.5 text-sm border border-border rounded bg-canvas outline-none focus:ring-2 focus:ring-brand w-52">
                        </div>
                    </div>
                    <div>
        ...
        <div class="bg-canvas rounded-xl border border-border shadow-sm overflow-hidden">
            <table class="w-full text-left">
                <thead class="bg-wash border-b border-border">
                    <tr>
                        <th class="px-6 py-4 text-[11px] font-bold uppercase tracking-wider text-muted">Columna 1</th>
                        <th class="px-6 py-4 text-[11px] font-bold uppercase tracking-wider text-muted">Columna 2</th>
                ...
                <tbody id="modulo-tbody" class="divide-y divide-border">
                    @forelse($registros as $r)
                    <tr class="hover:bg-gold/5 transition-colors cursor-pointer modulo-row"
                        data-id="{{ $r->id }}"
                        data-estado="{{ $r->estado }}"
                        data-texto="{{ strtolower($r->nombre ?? '') }}"
                        onclick="openPanel(this)">
                        <td class="px-6 py-4 text-sm font-medium">{{ $r->nombre }}</td>
                        <td class="px-6 py-4 text-sm text-muted">—</td>
                        <td class="px-6 py-4">
                    ...
                    @empty
                    <tr>
                        <td colspan="4" class="px-6 py-10 text-center text-muted text-sm">Sin registros</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
...
══════════════════════════════════════════════════════════════ --}}
<aside class="fixed top-0 right-0 h-screen w-[400px] bg-canvas shadow-2xl border-l border-gold z-[60] detail-panel closed flex flex-col"
       id="modulo-panel">

    {{-- Header fijo --}}
    <div class="p-6 border-b border-border bg-surface flex justify-between items-center shrink-0">
        <div>
            <p class="text-[11px] font-bold uppercase tracking-wider text-muted">Detalles</p>
            <h2 class="text-xl font-bold text-brand" id="panel-titulo">—</h2>
        </div>
        <button class="w-10 h-10 rounded-full hover:bg-wash flex items-center justify-center transition-colors"
                onclick="closePanel()">
    ...
    <div class="flex border-b border-border px-6 bg-canvas shrink-0">
        <button class="px-4 py-3 text-brand font-bold border-b-2 border-brand text-sm"
                onclick="switchPanelTab('info', this)">Información</button>
        <button class="px-4 py-3 text-muted font-medium text-sm hover:text-brand"
        ...
        <div class="p-6 space-y-6" id="tab-info">
            <section>
                ...
                <div class="grid grid-cols-2 gap-4 text-sm">
                    <div class="space-y-1">
                        <p class="text-[10px] text-muted font-medium">Campo 1</p>
                        <p class="font-semibold" id="panel-campo1">—</p>
                    </div>
                    <div class="space-y-1">
                        <p class="text-[10px] text-muted font-medium">Campo 2</p>
    ...
    <div class="p-6 border-t border-border bg-wash flex gap-3 shrink-0">
        <button class="flex-1 py-3 bg-canvas border border-gold text-brand font-bold rounded-lg text-sm hover:bg-gold/10 transition-colors">
            Editar
        </button>
        <button class="flex-1 py-3 bg-brand text-white font-bold rounded-lg text-sm hover:opacity-90 transition-opacity">
            Acción principal
        </button>
    </div>
</aside>
...
function switchPanelTab(tab, btn) {
    ...
    if (btn) btn.className = 'px-4 py-3 text-brand font-bold border-b-2 border-brand text-sm';
}
